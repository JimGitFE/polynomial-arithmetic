...
const polyReformat = (poly, formatType) => {
    ...
    else if (formatType ? formatType === enums_1.polyFormats.polyString : typeof poly === 'string') {
        return [poly, stringToCoef(poly), coefToExp(stringToCoef(poly))];
        // Coefficient Array Representation // [1,1,1,0,1,1,0,0,1,1]
    }
    else if (formatType ? formatType === enums_1.polyFormats.polyCoefficients : true) {
        return [coefToString(poly), poly, coefToExp(poly)];
        // Exponent Array Representation // [9,8,7,5,4,1,0]
    }
    else if (formatType === enums_1.polyFormats.polyExponents) { // isDescending(<polyExponents>poly) - [2,1,0] = (2x^2 +x +1 || x^2 + x + 1)
